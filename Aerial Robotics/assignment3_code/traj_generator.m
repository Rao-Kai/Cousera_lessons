...
% Constraint 1 ==> Pi(t=0) = wi for all i=1:n 即每段轨迹的起点位置约束 n条轨迹有n个
% Ex: P1(0) = w1, a11 = w1
...
% Constraint 2 ==> Pi(t=1) = wi+1 for all i=1:n 即每段轨迹的终点位置约束 n条轨迹有n个
...
% Constraint 3 ==> P1_k(t=0) = 0 for all k=1..3 (derivative) 即起点的高阶导数为0，有3个
% Ex: P1_1(t=0)=0, a12 = 0
...
% Constraint 4 ==> Pn_k(t=1) = 0 for all k=1..3 (derivative) 即终点的高阶导数为0，有3个
...
% 每段轨迹连续, 对于minimum snap, 7次多项式可以求导6次，n条轨迹有6*(n-1)个约束
% Ex: P1_1(t=1)-P2_1(t=0) = 0, a12 + 2a13 +…+7a18 - a22 = 0
% Ex: A(2*n+6+1,)=[0 1 2 3 4 5 6 7 0 -1 0 0 0 0 0 0 zeros]
% Ex: b(2*n+6+1)=0
for i=2:n
    for k=1:6
        A(2*n+6+(i-2)*6+k, (i-2)*8+1:((i-2)*8+16)) = [polyT(8,k,1) -polyT(8,k,0)];
    end
end
